...
function showPromoSwitchPannel(shown) {
    if (shown) {
        ...
        getPref(promoEnabled, true, function(value) {
...
function showControlPannel(compatibility, pid) {
    ...
    document.getElementById("enable-toolbar-switch").addEventListener('click', function(e) {
        ...
        if (curChecked) {
            ...
            chrome.runtime.sendMessage({text: Request.disable_toolbar_icon}, function(response) {
            });
        } else {
            setPref(Prop.enable_toolbar, true);
            chrome.runtime.sendMessage({text: Request.enable_toolbar_icon}, function(response) {
            });
        }
    ...
    } else if (isDealPriceAvailable(pid, getGlobalValue(GlovalKey.areacode))) {
...
function initLayout(response) {
    ...
    if (response.state == CompatibleState.none || response.state == CompatibleState.upgrade_env) { // CompatibleState.none is canonical plugin
        ...
        if (getUILang() != 'zh-CN') {
            ...
            if (true) { // 360safe don't support new features
                ...
                url360safe.onclick = function(e) {
                    chrome.tabs.create({ url: getUpgradeLink(ProductId.safe), selected: true });
                };
            }
        ...
        url360ts.onclick = function(e) {
            chrome.tabs.create({ url: getUpgradeLink(ProductId.ts), selected: true });
        };
        ...
        if (browserInfo.appname == 'opera') {
            url360ts.onclick = function(e) {
                chrome.tabs.create({ url: getOperaExtUrl(), selected: true });
            };
        } else if (browserInfo.appname == 'yandex') {
            url360ts.onclick = function(e) {
                chrome.tabs.create({ url: getYandexExtUrl(), selected: true });
            };
        } else if (browserInfo.appname == 'chrome') {
            url360ts.onclick = function(e) {
                chrome.tabs.create({ url: getChromeExtUrl(), selected: true });
            };
        }
